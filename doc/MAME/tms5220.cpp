...
{
	printf("\nRNG=%04X,sample=%d,m_IC=%d,m_PC=%d,cycle_%C,m_pitch_count=%d,m_pitch_zero=%d,m_inhibit=%d,m_OLDE=%d,m_OLDP=%d,m_DDIS=%d,m_SPEN=%d,m_TALK=%d,m_TALKD=%d,m_uv_zpar=%d ", m_RNG&0x1FFF, this_sample, m_IP,  m_PC, (m_subcycle==1)?'A':'B', m_pitch_count, m_pitch_zero, m_inhibit, m_OLDE, m_OLDP, m_DDIS, m_SPEN, m_TALK, m_TALKD, m_uv_zpar);
	...
	printf(", m_x=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",m_x[9],m_x[8],m_x[7],m_x[6],m_x[5],m_x[4],m_x[3],m_x[2],m_x[1],m_x[0]);

	printf("\ncurrent: %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
	m_current_energy, m_current_pitch, m_current_k[0],
	  m_current_k[1],  m_current_k[2], m_current_k[3],
	  m_current_k[4],  m_current_k[5], m_current_k[6],
	  m_current_k[7],  m_current_k[8], m_current_k[9]);

	printf("\ntarget : %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
	(m_coeff->energytable[m_new_frame_energy_idx]),
	(m_coeff->pitchtable[m_new_frame_pitch_idx]  ),
	(m_coeff->ktable[0][m_new_frame_k_idx[0]]    ),
	(m_coeff->ktable[1][m_new_frame_k_idx[1]]    ),
	(m_coeff->ktable[2][m_new_frame_k_idx[2]]    ),
	(m_coeff->ktable[3][m_new_frame_k_idx[3]]    ),
	...
	printf(" indexes: %d,%d,%d", m_new_frame_energy_idx, rep_flag, m_new_frame_pitch_idx);
	for (int k=0; k<10; k++) printf(",%d", m_new_frame_k_idx[k]);
}
			...
			else // Not a new frame, just interpolate the existing frame.
			{
				...
				if (m_subcycle == 2)
				{
					switch(m_PC)
					{
						...
						m_current_k[m_PC-2] = (m_current_k[m_PC-2] + (((m_coeff->ktable[m_PC-2][m_new_frame_k_idx[m_PC-2]] - m_current_k[m_PC-2])*(1-inhibit_state)) INTERP_SHIFT))*(1-(((m_PC-2)<4)?0:m_uv_zpar));
						break;
						case 12: /* PC = 12 */
...
//#endif
			...
			else // digital I/O pin output is 12 bits
			{
				...
				// N taps:                       ^^ ^^^            = 0x3E00;
				// output: ssss ssss ssss ssss snnn nnnn nnnN NNNN
				this_sample = (this_sample<<1)|((this_sample&0x3E00)>>9);
				buffer[buf_count] = this_sample;
#endif
			}
			// Update all counts
...
void tms5220_device::device_start()
{
	...
	*/
	...
	case TMS5220_IS_5220:
		...
		break;
	default:
		printf("Unknown variant in tms5220_set_variant\n");
	}
